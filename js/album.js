...
function onResizeAndLoad() {
	...
	$(window).resize(function() {
		...
		// including cookies
		$.cookie("width", getWidth());
		$.cookie("height", getHeight());
	});
}
...
function setContentPos() {
	...
	var factor = 0.6;
	var margin = (getWidth() - $(".albums").width() - $(".info").width()) * factor;
...
function setCaptionSize() {
	...
	var left	= Math.ceil((getWidth() - imgW) / 2);
	var top		= (getHeight() - imgH) / 2 + imgH - $("#caption").height();
